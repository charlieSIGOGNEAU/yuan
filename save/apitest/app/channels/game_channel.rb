class GameChannel < ApplicationCable::Channel
  def subscribed
    # Le client doit fournir un game_id et user_id pour s'abonner
    game_id = params[:game_id]
    user_id = params[:user_id]

    if game_id.blank? || user_id.blank?
      puts "❌ GameChannel: game_id ou user_id manquant"
      reject
      return
    end

    # Vérifier que l'utilisateur fait bien partie de cette partie
    ...
    unless @game_user
      puts "❌ GameChannel: Utilisateur #{user_id} ne fait pas partie de la partie #{game_id}"
      reject
      return
    end
